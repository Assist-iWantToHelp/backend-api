  ...
  class UsersApi < Grape::API
    ...
    resource :users do
      desc 'Get my profile' do
        tags %w[users]
        http_codes [
          { code: 200, model: Entities::Volunteer, message: 'Profile overview' }
        ]
      end
      get do
        profile = current_user
        present profile, with: Entities::Volunteer
      end

      desc 'Update profile' do
        tags %w[users]
        http_codes [
          { code: 200, model: Entities::Volunteer, message: 'Profile update' },
